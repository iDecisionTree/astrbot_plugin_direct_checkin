...
| 用例 | 操作 | 预期 |
|---|---|---|
| 6.1 每周计分槽 | 在 3 个不同自然日各提交 1 份有效 | 前 2 份 `VALID_COUNTED`，第 3 份 `VALID_EXTRA` |
| 6.2 每日只计一次 | 同一天提交 2 份有效 | 第 1 份计次；第 2 份回复“今天已经计过一次啦…”，状态 `VALID_EXTRA` |
| 6.3 人工共享名额 | 管理员 `/d add <学号>`，本周未满 | 占用一个计分槽，本周有效计次 +1 |
| 6.4 人工超额 | 本周已满时再 `/d add <学号>` | 回复“本周已满 2/2，这次记为额外…”，不计入每周次数 |
| 6.5 人工扣减 | 本周计次 >0 时 `/d remove <学号>` | 本周有效计次 -1 |
| 6.6 人工扣减到 0 | 本周为 0 时 `/d remove` | 拒绝：`当前周计次已经是 0…` |
| 6.7 人工先占槽 | 先 `/d add`，再当天自动打卡 | 人工占槽，自动打卡按剩余名额处理 |
| 6.8 边界-周一 | 周日 23:59 与周一 00:00 分属两周 | 以北京时间周键区分 |
...
| 9.6 累计有效计分次数 | 查看 `用户汇总` 的该列 | = 全部 AI 通过提交 + 人工调整净和（含额外）|
| 9.7 累计完成周数 | 查看 `用户汇总` 的该列 | 等于该用户有效计次达标的自然周数（人工计入）|
| 9.8 格式 | 查看表头 | 首行冻结、自动筛选、列宽合理、时间为北京时间 |
| 9.9 隐私 | 检查文件 | 明细不含 Word 正文全文 |
| 9.10 临时清理 | 等待 `export_keep_minutes` | `exports/` 下文件被清理 |
