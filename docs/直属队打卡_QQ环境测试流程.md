...
| 9.6 累计完成周数 | 查看 `用户汇总` 的“累计完成周数”列 | 等于该用户有效计次达标的自然周数（人工调整计入）|
| 9.7 格式 | 查看表头 | 首行冻结、自动筛选、列宽合理、时间为北京时间 |
| 9.8 隐私 | 检查文件 | 明细不含 Word 正文全文 |
| 9.9 临时清理 | 等待 `export_keep_minutes` | `exports/` 下文件被清理 |
