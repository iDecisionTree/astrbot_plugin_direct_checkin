> **版本说明（v1.1，2026-09-19）**：下文保留 v1 设计过程作为历史记录，旧条文不再覆盖现行实现。现行规则以 [用户文档](直属队打卡_用户使用文档.md) 与 [代码检查与改版说明](代码检查与改版说明.md) 为准。
>
> v1.1 明确变更：周目标持久化、历史补 2、当周冻结；自动每日最多一次；高相似附差异交 AI；累计周有效计次与有效材料／人工调整分列；统计分类改为已完成／进行中／未开始；Excel 为五张表；NAS 全清改为暂存、数据库提交标记与失败恢复；消息采用完整事件键幂等；数据库 schema 版本为 2。历史关于“所有通过材料都算累计计分”、固定四表、固定 0/1/2 分类及不可恢复删除的描述均已替代。
...
3. 配置项 `bootstrap_admin_qq` 对应的 QQ 为**超级管理员**，任何情况下都不能被 `/d admin remove` 移除。
4. 管理员可以移除自己，但系统必须保证操作完成后至少保留 1 名管理员；否则拒绝删除，避免插件永久失管。
...
| `/d reset` | 显示重置确认提示 |
| `/d reset confirm` | 第一次返回二次确认；连续第二次才执行。执行时清空打卡相关数据库（用户绑定、提交、人工调整、暂停、审计）并删除 NAS 归档目录 `nas_base_dir` 下的全部文件，保留管理员表与归档根目录 |
...
每位用户每周最多有 2 个计分槽，**由自动打卡与人工 `/d add` 共同占用**。

自动打卡（AI 审查通过后）：

- 同一北京自然日最多计 1 次；若当天已有计次，本次状态为 `VALID_EXTRA`，材料有效但不增加本周计次；
- 若本周已有计次（自动计次 + 人工计次 - 人工扣减）`< 2`：状态为 `VALID_COUNTED`，占用一个计分槽；
- 若 `>= 2`：状态为 `VALID_EXTRA`，材料有效但不增加本周计次。

人工 `/d add`：

- 本周仍有空槽时占用一个槽（`counted=1`）；
- 本周已满时记为额外（`counted=0`），不计入每周次数。

### 14.3 人工调整

管理员 `/d add`、`/d remove` 不直接伪造提交记录，而是写入独立 `count_adjustments` 表，并记录是否占用计分槽（`counted` / `counted_slot`）。

当前周有效计次：

```text
有效计次 = 自动 VALID_COUNTED 数 + 人工计次槽数 - 人工扣减次数
展示计次 = clamp(有效计次, 0, 2)
完成状态 = 展示计次 >= 2
```
...
```text
累计有效计分次数 = 所有 AI 通过的提交数 + 人工调整 delta 净和
```

- 包含 `VALID_COUNTED` 与 `VALID_EXTRA`（同一天多份、本周超 2 份的有效提交都计入）；
- 人工 `/d remove`（delta=-1）按净额扣减；
- 结果不小于 0。
...
- 对 `skip d`：恢复后当天可继续打卡。
- 对 `skip w`：恢复后**本周不再豁免**，重新按正常周要求完成 2 次打卡；因此 `/d stat`、`/d get` 也不再显示“暂停周”。
- `resume` 会一次性结束所有仍在生效中的暂停记录（day/week 均结束）。
- 暂停期间已经产生的提交仍保留，不删除。
...
- 解析目标用户。
- 给“当前北京时间周”写入 `delta=+1`，并与自动打卡共享每周 2 个计分槽：
  - 本周仍有空槽：`counted=1`，占用槽位；
  - 本周已满：`counted=0`，记为额外，不计入每周次数。
- 返回目标姓名、学号和调整后的本周有效计次。
- 必须记录操作管理员与时间。

### 16.2 `/d remove <学号/QQ>`

- 给当前周写入 `delta=-1`，作为扣减计入本周有效计次。
- 不允许本周展示计次小于 0；如果已经为 0，拒绝操作。
- 不删除原提交，不改 AI 审查结果。
...
- 累计额外有效提交数
- 累计无效提交数
- 累计有效计分次数（全部 AI 通过提交 + 人工调整净和，额外提交也计入）
- 累计完成周数（按周统计有效计次 >= 每周应完成次数的周数，人工计入）
- 最后打卡时间
...
- AI 通过且本周 0 次：变 1/2。
- AI 通过且本周 1 次：变 2/2。
- AI 通过且本周 2 次（自动或人工占满）：`VALID_EXTRA`，仍保持 2/2。
- 同一天第二份有效材料：`VALID_EXTRA`，不增加计次。
- AI 不通过：不增加计次。
...
- 同一自然日只计 1 次；不同自然日才可分别计次。
- 人工 `/d add` 不受每日限制，仅受每周 2 次上限约束。
